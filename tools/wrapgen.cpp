/*
 * Standalone wrapgen for aarch64.
 *
 * Based on Darling's wrapgen, modified to accept EM_AARCH64 ELF files
 * in addition to EM_X86_64.
 *
 * Parses an ELF shared library and generates C wrapper source code
 * that uses elf_calls to redirect symbols to the native ELF library.
 */
...
void parse_elf(const char* elf, std::string& soname, std::set<std::string>& symbols, std::set<std::string>& vars)
{
	...
	/* Accept both x86-64 and aarch64 ELF files */
	if (ehdr->e_machine != EM_X86_64 && ehdr->e_machine != EM_AARCH64)
	{
		std::stringstream ss;
		ss << elf << " is not an ELF for x86-64 or aarch64 (e_machine=" << ehdr->e_machine << ")";
		throw std::runtime_error(ss.str());
	}
	...
	for (int i = 0; i < ehdr->e_phnum; i++)
	{
		...
		if (phdr->p_type == PT_DYNAMIC)
		{
			...
			for (int j = 0; j < (int)phdr->p_filesz; j += sizeof(Elf64_Dyn))
			{
				const Elf64_Dyn* dyn;
...
end_dyn:
	...
	if (strings != NULL)
	{
		...
		for (int i = 0; i < ehdr->e_shnum; i++)
		{
			...
			{
				for (int j = 0; j < (int)shdr->sh_size; j += sizeof(Elf64_Sym))
				{
					const Elf64_Sym* sym;
